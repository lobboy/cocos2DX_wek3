...
bool Scene101::init()
{
	...
	//bean01->setPosition(Vec2(origin.x + 300, origin.y + 550)); // 位置通常放置在螢幕正中間
	//this->addChild(bean01, 2);

	char buffer[100] = "";
	sprintf(buffer, "%3.4f", visibleSize.width);
	...
	// create and initialize a label, add a label shows "your mother was a murloc"
	auto label87 = Label::createWithTTF("Your mother was a murloc!", "fonts/Marker Felt.ttf", 32);
	label87->setAlignment(cocos2d::TextHAlignment::CENTER); // 預設靠左對齊
	label87->setWidth(400);	// 設定每行文字的顯示寬度
	size = label87->getContentSize();
	label87->setPosition(Vec2(origin.x + visibleSize.width / 2 - size.width / 2, origin.y + visibleSize.height / 2 - size.height / 2 - 10));
	this->addChild(label87, 1);
	...
	label4->setColor(Color3B(250, 251, 170));
	size = label4->getContentSize();
	label4->setPosition(Vec2(origin.x + visibleSize.width / 2, origin.y + visibleSize.height - 260 - size.height));
	...
	// add Mr.Bean Button
	this->Beanbtn = Sprite::create("scene101/bean01.png");
	size = Beanbtn->getContentSize();
	this->Beanbtn->setPosition(Vec2(origin.x + 300, origin.y + 550));
	pos = Beanbtn->getPosition();
	this->rectBean = Rect(pos.x - size.width / 2, pos.y - size.height / 2, size.width, size.height);
	this->addChild(Beanbtn, 2);
...
bool  Scene101::onTouchBegan(cocos2d::Touch *pTouch, cocos2d::Event *pEvent)//觸碰開始事件
{
	...
	if (rectCuber.containsPoint(touchLoc)) {
		...
		cuberbtn->setTexture("scene101/cuberbtn2.png");
	}
	if (rectBean.containsPoint(touchLoc)) {
		drag = true;
		lastMousePosi = touchLoc;
		Beanbtn->setTexture("scene101/bean03.png");
		unscheduleAllCallbacks();  // 將所有註冊在 Schedule 中的程序都取消
	}
...
void  Scene101::onTouchMoved(cocos2d::Touch *pTouch, cocos2d::Event *pEvent) //觸碰移動事件
{
	...
	if (drag)
	{
		Mousemovept = touchLoc - lastMousePosi;
		lastMousePosi = touchLoc;
		Point beanMov = Beanbtn->getPosition();
		beanMov += Mousemovept;
		Beanbtn->setPosition(beanMov);

		Size size = Beanbtn->getContentSize();
		Point pos = Beanbtn->getPosition();
...
void  Scene101::onTouchEnded(cocos2d::Touch *pTouch, cocos2d::Event *pEvent) //觸碰結束事件 
{
	...
	if (rectCuber.containsPoint(touchLoc)) {
		unscheduleAllCallbacks();  // 將所有註冊在 Schedule 中的程序都取消
		cuberbtn->setTexture("scene101/cuberbtn1.png");
	}

	if (rectBean.containsPoint(touchLoc)) {
		unscheduleAllCallbacks();  // 將所有註冊在 Schedule 中的程序都取消
		Beanbtn->setTexture("scene101/bean01.png");
	}
