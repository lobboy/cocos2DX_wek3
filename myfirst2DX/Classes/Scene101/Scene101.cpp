...
bool Scene101::init()
{
	...
	bean01->setColor(Color3B(100,256,128));
	bean01->setPosition(Vec2(330, 593)); // 位置通常放置在螢幕正中間
	bean01->setRotation(75);
	...
	auto label1 = Label::createWithBMFont("fonts/obov.fnt", "Scene 101");
	...
	std::string str1 = strings["chinese3"].asString();
	std::string str2 = strings["chinese4"].asString();
	std::string str3 = strings["chinese2"].asString();
	auto label2 = Label::createWithBMFont("fonts/UKgood.fnt", str1);
	auto label3 = Label::createWithBMFont("fonts/UKgood.fnt", str2);
	auto label4 = Label::createWithBMFont("fonts/hansans.fnt", str3);
	...
	label4->setColor(Color3B(128, 100, 255));
	label4->setPosition(Vec2(origin.x + visibleSize.width / 2, origin.y + visibleSize.height - 200 - label->getContentSize().height));
